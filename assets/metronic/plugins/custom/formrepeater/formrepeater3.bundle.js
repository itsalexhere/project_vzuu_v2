...
(function ($) {
  ...
  $.fn.repeater = function (fig) {
    ...
    $(this).each(function () {
      ...
      var setIndexes = function ($items, groupName, repeaters, act = null) {
        $items.each(function (index) {
          ...
          $filterNested($item.find("[name]"), repeaters).each(function () {
            ...
            var newNameID = name + "_" + index;
            $input.attr("name", newName);
            $input.attr("id", newNameID);
      ...
      setIndexes($items(), getGroupName(), fig.repeaters, "list");
      initNested($items());
      if (fig.initEmpty) {
        $items().remove();
      }
      ...
      var appendItem = (function () {
        ...
        return function ($item, data) {
          $list.append($item);
          setIndexes($items(), getGroupName(), fig.repeaters, "add");
      ...
      var selectItem = function () {
        if (del_id.length > 0) {
          for (let index = 0; index < del_id.length; index++) {
            $("#select_brand_id_" + del_id[index]).select2({
              placeholder: "--Choose Options--",
              allowClear: true,
            });
            $("#select_type_ownership_" + del_id[index]).select2({
              placeholder: "--Choose Options--",
      ...
      $list.on("click", "[data-repeater-delete]", function () {
        ...
        var del_idx = $(this).find("input")[0];
        if (del_idx !== undefined) {
          var arr1 = del_idx.id.split("_")[2];
          del_id.push(parseInt(arr1));
        }
